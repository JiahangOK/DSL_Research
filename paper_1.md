...
一个测试工具需要识别app中的部件，积累测试过程中需要的知识。我们并不能用一个对象的内存地址来代表一个部件，因为一个GUI可能会被创建和销毁很多次。在一个GUI树中，给定部件$w_n$，节点路径$w=<w_1,w_2,...,w_n>$是从$w_1$到$w_n$的一个遍历路径，和文件系统中路径的概念类似，如果$w_1$是GUI树的根节点，则$w$是一个绝对路径，如果不是GUI树的根节点，则$w$是一个相对路径。
